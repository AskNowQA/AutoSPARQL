...
public class SPARQLEndpointMetrics {
	...
	public SPARQLEndpointMetrics(SparqlEndpoint endpoint, Connection connection) {
	...
	private void createDatabaseTables(){
		try {
			...
			stmt.execute("CREATE TABLE IF NOT EXISTS CLASS_PREDICATE_OCCURRENCES(CLASS VARCHAR(1000), PREDICATE VARCHAR(1000), OCCURRENCES INTEGER)");
	...
	public double getGoodness(NamedClass subject, ObjectProperty predicate, Individual object){
		...
		double goodness = Double.MIN_VALUE;
		//get all asserted classes of object and get the highest value
		//TODO inference
...
//		pmiGen.precompute(Arrays.asList(new String[]{"http://dbpedia.org/ontology/"}));
		...
		System.out.println(pmiGen.getGoodness(film, pStarring, bradPitt));
